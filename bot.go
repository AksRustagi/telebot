...
func NewBot(token string) (*Bot, error) {
	opts := BotOptions{
		Server: "https://api.telegram.org/bot%s/%s",
		FileUrl: "https://api.telegram.org/file/bot%s/%s",
	}
	user, err := getMe(token, opts)
	if err != nil {
		return nil, err
	}
...
// Same as NewBot but with options field
// Server field could be used to change Telegram server url
func NewCustomBot(token string, opts BotOptions) (*Bot, error) {
	user, err := getMe(token, opts)
	if err != nil {
		return nil, err
	}
...
// SendCustomRequest sends some custom event to the server
func (b *Bot) SendCustomRequest(customMethod string, parameters string, options *SendOptions) error {
	...
	responseJSON, err := sendCommand("sendCustomRequest", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) SendMessage(recipient Recipient, message string, options *SendOptions) error {
	...
	responseJSON, err := sendCommand("sendMessage", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) ForwardMessage(recipient Recipient, message Message) error {
	...
	responseJSON, err := sendCommand("forwardMessage", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) SendPhoto(recipient Recipient, photo *Photo, options *SendOptions) error {
	...
	if photo.Exists() {
		...
		responseJSON, err = sendCommand("sendPhoto", b.Token, params, b.Options)
	} else {
		responseJSON, err = sendFile("sendPhoto", b.Token, "photo",
			photo.filename, photo.FileBytes, params, b.Options)
	}
...
func (b *Bot) SendAudio(recipient Recipient, audio *Audio, options *SendOptions) error {
	...
	if audio.Exists() {
		...
		responseJSON, err = sendCommand("sendAudio", b.Token, params, b.Options)
	} else {
		responseJSON, err = sendFile("sendAudio", b.Token, "audio",
			audio.filename, audio.FileBytes, params, b.Options)
	}
...
func (b *Bot) SendDocument(recipient Recipient, doc *Document, options *SendOptions) error {
	...
	if doc.Exists() {
		...
		responseJSON, err = sendCommand("sendDocument", b.Token, params, b.Options)
	} else {
		responseJSON, err = sendFile("sendDocument", b.Token, "document",
			doc.filename, doc.FileBytes, params, b.Options)
	}
...
func (b *Bot) SendSticker(recipient Recipient, sticker *Sticker, options *SendOptions) error {
	...
	if sticker.Exists() {
		...
		responseJSON, err = sendCommand("sendSticker", b.Token, params, b.Options)
	} else {
		responseJSON, err = sendFile("sendSticker", b.Token, "sticker",
			sticker.filename, sticker.FileBytes, params, b.Options)
	}
...
func (b *Bot) SendVideo(recipient Recipient, video *Video, options *SendOptions) error {
	...
	if video.Exists() {
		...
		responseJSON, err = sendCommand("sendVideo", b.Token, params, b.Options)
	} else {
		responseJSON, err = sendFile("sendVideo", b.Token, "video",
			video.filename, video.FileBytes, params, b.Options)
	}
...
func (b *Bot) SendLocation(recipient Recipient, geo *Location, options *SendOptions) error {
	...
	responseJSON, err := sendCommand("sendLocation", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) SendVenue(recipient Recipient, venue *Venue, options *SendOptions) error {
	...
	responseJSON, err := sendCommand("sendVenue", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) SendChatAction(recipient Recipient, action ChatAction) error {
	...
	responseJSON, err := sendCommand("sendChatAction", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
// SendGame sends a custom message with game to recipient
// Game should be created with BotFather bot and attached to the bot
func (b *Bot) SendGame(recipient Recipient, gameName string, options *SendOptions) (*Message, error) {
	params := map[string]string{
		"chat_id": recipient.Destination(),
	...
	responseJSON, err := sendCommand("sendGame", b.Token, params, b.Options)
	if err != nil {
		return nil, err
	}
...
func (b *Bot) Respond(query Query, results []Result) error {
	...
	responseJSON, err := sendCommand("answerInlineQuery", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) AnswerInlineQuery(query *Query, response *QueryResponse) error {
	...
	responseJSON, err := sendCommand("answerInlineQuery", b.Token, response, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) AnswerCallbackQuery(callback *Callback, response *CallbackResponse) error {
	...
	responseJSON, err := sendCommand("answerCallbackQuery", b.Token, response, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) GetFile(fileID string) (File, error) {
	...
	responseJSON, err := sendCommand("getFile", b.Token, params, b.Options)
	if err != nil {
		return File{}, err
	}
...
func (b *Bot) LeaveChat(recipient Recipient) error {
	...
	responseJSON, err := sendCommand("leaveChat", b.Token, params, b.Options)
	if err != nil {
		return err
	}
...
func (b *Bot) GetChat(recipient Recipient) (Chat, error) {
	...
	responseJSON, err := sendCommand("getChat", b.Token, params, b.Options)
	if err != nil {
		return Chat{}, err
	}
...
func (b *Bot) GetChatAdministrators(recipient Recipient) ([]ChatMember, error) {
	...
	responseJSON, err := sendCommand("getChatAdministrators", b.Token, params, b.Options)
	if err != nil {
...
func (b *Bot) GetChatMembersCount(recipient Recipient) (int, error) {
	...
	responseJSON, err := sendCommand("getChatMembersCount", b.Token, params, b.Options)
	if err != nil {
		return 0, err
	}
...
func (b *Bot) GetUserProfilePhotos(recipient Recipient) (UserProfilePhotos, error) {
	...
	responseJSON, err := sendCommand("getUserProfilePhotos", b.Token, params, b.Options)
	if err != nil {
...
func (b *Bot) GetChatMember(recipient Recipient, user User) (ChatMember, error) {
	...
	responseJSON, err := sendCommand("getChatMember", b.Token, params, b.Options)
	if err != nil {
		return ChatMember{}, err
	}
...
func (b *Bot) GetFileDirectURL(fileID string) (string, error) {
	...
	return fmt.Sprintf(b.Options.FileUrl, b.Token, f.FilePath), nil
}
